import random
from users.models.email_models import EmialCodeModel
...
def send_code_to_email(user, purpose):
    ...
    last_code = EmialCodeModel.objects.filter(user=user, purpose=purpose).order('-created_at').first()
    ...
    EmialCodeModel.objects.create(user=user, code=raw_code, purpose=purpose) # کد رو میسازه و توی دیتابیس ذخیره میکنه
