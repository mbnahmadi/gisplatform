...
def verify_user_email_code(user, code, purpose):
    """
    ...
    with transaction.atomic():
        ...
        # raw_code.save(update_fields=['attempts'])
            ...
            #getattr(settings, 'VERIFY_CODE_MAX_ATTEMPTS', 3) # از ستینگ بیا VERIFY_CODE_MAX_ATTEMPTS رو بگیر اگه نداشت 3 بذار
...
def verify_user_mobile_2FA_code(user, code, mobile, purpose):
    ...
    with transaction.atomic():
        ...
        # بررسی انقضا
            ...
            TwoFAModels.objects.filter(pk=raw_code.pk).update(attempts=F('attempts') + 1)
